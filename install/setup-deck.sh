...
sanity_check() {
  ...
  if [[ -d "${HOME}/Emulation" ]]; then
    log "  EmuDeck root detected at ${HOME}/Emulation"
  elif [[ -d /run/media/mmcblk0p1/Emulation ]]; then
    log "  EmuDeck root detected on SD: /run/media/mmcblk0p1/Emulation"
  else
    die "EmuDeck not detected. Install EmuDeck first (https://www.emudeck.com)."
  fi
...
write_config() {
  ...
  local detect="$(${USER_BIN}/retrosync --config /dev/null deck detect-paths --system snes 2>/dev/null || true)"
  local saves_root="${HOME}/Emulation/saves/retroarch/saves"
  local roms_root="${HOME}/Emulation/roms/snes"
