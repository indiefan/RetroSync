...
install_retrosync_app() {
  ...
  # OAuth config in /home/retrosync/.config/rclone/. Rather than open up
  # those files (the rclone config holds a refresh token), install a
  # wrapper that auto-elevates to the retrosync user. Operators who are
...
#!/usr/bin/env bash
# RetroSync CLI wrapper — elevates to the retrosync system user so the
# daemon's config + rclone credentials are readable. Generated by setup.sh;
# do not edit by hand (will be overwritten on upgrade).
set -e
